...
export default function QuickApply() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const text = `*Quick Apply - S.H.Q Enterprises*%0A%0A_Name:_ ${encodeURIComponent(name)}%0A_Phone:_ ${encodeURIComponent(phone)}%0A_Category:_ ${encodeURIComponent(category)}`
        ...
        <div className="mb-3 bg-white rounded-2xl shadow-2xl p-5 w-72 md:w-80 border border-primary/20 animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <input type="text" placeholder="Your Name" required value={name}
              ...
              className="w-full px-3 py-2.5 rounded-lg border border-primary/20 text-sm outline-none focus:border-secondary bg-[#E6F3FB] text-ink" />
