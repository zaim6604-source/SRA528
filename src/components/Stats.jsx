...
function AnimatedCounter({ end, suffix = '', label, icon }) {
  ...
  return (
    <div ref={ref} className="text-center p-6">
      <i className={`fas ${icon} text-3xl text-accent mb-3`} />
    ...
    <section className="bg-gradient-to-r from-primary to-highlight py-12 lg:py-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <AnimatedCounter icon="fa-building" end={150} suffix="+" label="Partner Employers" />
          <AnimatedCounter icon="fa-globe-asia" end={9} label="Countries" />
          <AnimatedCounter icon="fa-calendar-check" end={12} suffix="+" label="Years Experience" />
        </div>
      </div>
    </section>
  );
